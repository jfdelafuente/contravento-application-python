...
export async function likeTrip(tripId: string): Promise<LikeResponse> {
  const response = await api.post<{success: boolean; data: LikeResponse; error: null}>(`/trips/${tripId}/like`);
...
export async function unlikeTrip(tripId: string): Promise<UnlikeResponse> {
  const response = await api.delete<{success: boolean; data: UnlikeResponse; error: null}>(`/trips/${tripId}/like`);
  return response.data.data;
}
...
export async function getTripLikes(
  ...
  const response = await api.get<{success: boolean; data: LikesListResponse; error: null}>(`/trips/${tripId}/likes`, {
    params: { page, limit },
  });
  return response.data.data;
}
