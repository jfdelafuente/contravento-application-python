...
> For frontend deployment details, see [Frontend Deployment section](#35-frontend-deployment) and [QUICK_START.md - Frontend Deployment](../../QUICK_START.md#frontend-deployment)
...
**Overview**: ContraVento frontend (React + TypeScript + Vite) can be deployed in multiple environments with different configurations.

> **📖 Complete frontend deployment guide**: See [QUICK_START.md - Frontend Deployment](../../QUICK_START.md#frontend-deployment) for detailed instructions.

#### 3.5.1 Frontend Deployment Modes

| Environment | Build Type | Server | Hot Reload | Source Maps | Access |
|-------------|------------|--------|------------|-------------|--------|
| **SQLite Local** | Dev (Vite) | Vite dev server | ✅ | ✅ | http://localhost:5173 |
| **Docker Full** | Dev (Vite) | Vite dev server | ✅ | ✅ | http://localhost:5173 |
| **Staging** | Production | Nginx (static) | ❌ | ✅ | https://staging.contravento.com |
| **Production** | Production | Nginx (static) | ❌ | ❌ | https://contravento.com |

#### 3.5.2 Quick Commands

**Development Mode (SQLite Local - No Docker):**
```bash
# Start frontend only
cd frontend
npm run dev
...
**Production Build (Staging/Production):**
```bash
# Build is automatic when deploying
./deploy.sh staging                      # Builds with source maps
./deploy.sh prod                         # Builds without source maps
...
|----------|-------------|---------|------------|
| `VITE_API_URL` | http://localhost:8000 | https://staging.contravento.com | https://api.contravento.com |
| `VITE_ENV` | development | staging | production |
| `VITE_DEBUG` | true | false | false |
| `VITE_TURNSTILE_SITE_KEY` | 1x00000000000000000000AA (test key) | Real site key | Real site key |

**Configuration files:**
- `.env.development` - SQLite Local defaults
- `.env.staging` - Staging-specific values
- `.env.production` - Production-specific values

#### 3.5.4 Production Build Optimizations

When deploying to staging/production, the frontend build includes:

- ✅ **Terser minification** - Reduces JavaScript size by ~70%
- ✅ **CSS minification** - Removes whitespace and comments
- ✅ **Tree shaking** - Eliminates unused code
- ✅ **Code splitting** - Separate vendor chunks (react-vendor, form-vendor, map-vendor)
- ✅ **Content hashing** - Cache-busting filenames (e.g., `index-abc123.js`)
- ✅ **Gzip compression** - Nginx compresses all text assets
- ✅ **Cache headers** - 1 year cache for assets, no-cache for index.html
...
dist/
├── assets/
│   ├── index-abc123.js           # Main app bundle (~400 KB)
│   ├── react-vendor-def456.js    # React libraries (~200 KB)
│   ├── form-vendor-ghi789.js     # Form libraries (~150 KB)
│   ├── map-vendor-jkl012.js      # Map libraries (~250 KB)
│   └── index-mno345.css          # Compiled styles (~50 KB)
└── index.html                     # Entry point (~2 KB)

Total: ~1.05 MB (uncompressed), ~350 KB (gzipped)
```

#### 3.5.5 Frontend + Backend Integration

**CORS Configuration:**

Backend automatically allows frontend origins based on `CORS_ORIGINS` environment variable:

```bash
# .env.local (Docker Full)
...
Frontend uses `VITE_API_URL` to determine backend location:
- Development: Direct connection to `http://localhost:8000`
- Staging/Production: HTTPS connection via configured domain

**Authentication Flow:**

Frontend uses **HttpOnly cookies** for authentication:
1. User logs in → Backend sets HttpOnly cookie
2. Frontend makes API requests → Cookie sent automatically
3. Backend validates cookie → Returns user data
4. No token storage in localStorage (XSS protection)

#### 3.5.6 Frontend Healthchecks

Each deployment mode includes frontend healthchecks:

**Docker Full (Vite dev server):**
```yaml
healthcheck:
...
#### 3.5.7 Troubleshooting Frontend Deployment

See [QUICK_START.md - Troubleshooting](../../QUICK_START.md#troubleshooting) for complete troubleshooting guide.
...
2. **"Failed to fetch" errors**
   - Verify `VITE_API_URL` matches running backend
   - Check backend is running and accessible

3. **Frontend not loading in Docker Full**
   - Ensure `--with-frontend` flag is used: `./deploy.sh local --with-frontend`
   - Check `docker ps` shows frontend container with status "Up"

4. **Hot reload not working**
   - Verify volume mounts in `docker-compose.local.yml`
   - Check Vite dev server logs: `docker logs contravento-frontend-local`

5. **Production build too large**
   - Expected: ~1 MB uncompressed, ~350 KB gzipped
   - Check for accidentally imported dev dependencies
...
| **postgres** | ✅ | ✅ | ✅ | ✅ | ✅ |
| **backend** | ✅ | ✅ | ✅ | ✅ | ✅ (3 replicas) |
| **frontend** | ❌ | ⚙️ Optional | ⚙️ Optional | ✅ | ✅ (3 replicas) |
| **redis** | ❌ | ✅ | ✅ | ✅ | ✅ |
| **mailhog** | ❌ | ✅ | ❌ | ❌ | ❌ |
| **pgadmin** | ❌ | ✅ | ❌ | ❌ | ❌ |
| **nginx** | ❌ | ❌ | ✅ | ✅ | ✅ |
...
> **Note**: ⚙️ Optional means service is available but disabled by default. Enable with `--with-frontend` flag.
