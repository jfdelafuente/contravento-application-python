...
- ⚡ **Instant startup** - No Docker container overhead
- 🎯 **Zero configuration** - Automatic setup with `--setup` flag
- 🔄 **Hot reload** - Backend and frontend auto-reload on code changes
- 💻 **Cross-platform** - Works on Windows, macOS, Linux
- 📦 **Minimal disk space** - ~50 MB (vs. ~500 MB for Docker)
...
1. ✅ Copies `backend/.env` from `backend/.env.dev.example`
2. ✅ Generates random `SECRET_KEY` for JWT tokens
3. ✅ Installs Python dependencies with `poetry install`
4. ✅ Creates SQLite database with Alembic migrations
5. ✅ Creates test users (admin, testuser, maria_garcia)
6. ✅ Seeds data (achievements, cycling types)
...
2. Edit code:
   - **Frontend changes** (`frontend/src/*.tsx`, `*.css`):
     - Save file → Browser updates in <2 seconds **without page reload** (HMR)
   - **Backend changes** (`backend/src/*.py`):
     - Save file → Server restarts in ~5-10 seconds
...
**What happens**:
1. Asks for confirmation (⚠️ **deletes all data**)
...
│   Database                                      │
│   SQLite 3 - File-based                         │
│   • Location: backend/contravento_dev.db        │
│   • Async access with aiosqlite                 │
│   • Migrations: Alembic                         │
└─────────────────────────────────────────────────┘
```
...
2. **Vite Proxy** (automatic)
   - Rewrites `/api/trips` → `http://localhost:8000/api/trips`
...
│   ├── contravento_dev.db   # SQLite database (created by --setup)
│   └── pyproject.toml       # Poetry dependencies
├── frontend/
│   ├── src/                 # React source code
│   ├── .env.development     # Frontend env (created by --setup)
│   ├── vite.config.ts       # Vite configuration (proxy settings)
│   └── package.json         # npm dependencies
├── run-local-dev.sh         # Bash startup script
└── run-local-dev.ps1        # PowerShell startup script
```
...
When you need PostgreSQL testing:
    → local-minimal (Docker + PostgreSQL only)
    ↓
When you need email/cache testing:
    → local-full (Docker + PostgreSQL + MailHog + Redis)
    ↓
