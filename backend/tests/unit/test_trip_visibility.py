...
class TestProfileServiceTripVisibility:
    ...
    async def test_update_trip_visibility_invalid_value_raises_error(
    ...
    async def test_update_trip_visibility_with_other_fields(
        self, db_session: AsyncSession, test_user
    ):
    ...
    async def test_update_trip_visibility_without_trip_visibility_keeps_current(
        self, db_session: AsyncSession, test_user
    ):
    ...
    async def test_update_both_visibilities_together(self, db_session: AsyncSession, test_user):
        ...
        update_data = ProfileUpdateRequest(
            profile_visibility="private", trip_visibility="followers"
        )
        result = await profile_service.update_profile(test_user.username, update_data)
